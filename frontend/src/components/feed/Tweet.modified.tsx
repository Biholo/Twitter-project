...
import { useLikeTweet, useUnlikeTweet, useBookmarkTweet, useUnbookmarkTweet, useRetweetTweet, useUnretweetTweet } from "@/api/queries/tweetQueries"
...
export function Tweet({ tweet }: { tweet: TweetType }) {
  ...
  const { mutate: retweetTweet, isPending: isRetweetPending } = useRetweetTweet();
  const { mutate: unretweetTweet, isPending: isUnretweetPending } = useUnretweetTweet();
  const navigate = useNavigate();

  // Déterminer si ce tweet est un retweet
  const isRetweet = tweet.tweet_type === 'retweet';
  const isRetweeting = isRetweetPending || isUnretweetPending;
  const isLiking = isLikePending || isUnlikePending;
  const isBookmarking = isBookmarkPending || isUnbookmarkPending;

  const openTweetDetails = (e: React.MouseEvent) => {
    // Empêcher la propagation si on clique sur un bouton d'action
  ...
  const openImageInFullScreen = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <Card className="overflow-hidden bg-white/80 backdrop-blur-sm dark:bg-gray-800/80 border-none">
      {/* Afficher qui a retweeté si c'est un retweet */}
      {isRetweet && (
        <div className="flex items-center px-4 pt-2 text-sm text-gray-500">
          <Repeat2 className="h-3.5 w-3.5 mr-2 text-gray-500" />
          <span>Retweeté par <span className="font-medium">{tweet.author.username}</span></span>
        </div>
      )}
      
      <CardHeader className="flex flex-row items-start gap-4 space-y-0 p-4" onClick={openTweetDetails}>
        <Avatar>
      ...
      <CardFooter className="border-t p-2">
        <div className="flex w-full justify-between">
          ...
          >
            <Repeat2 className={`h-4 w-4 ${isRetweeting ? 'animate-spin' : ''}`} />
          ...
          >
            <Bookmark className={`h-4 w-4 ${tweet.is_saved ? 'fill-blue-500' : ''} ${isBookmarking ? 'animate-pulse' : ''}`} />
